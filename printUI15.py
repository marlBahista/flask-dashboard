...
PRINTER_NAME = "EPSON L4160 Series"
BW_COST_PER_PAGE = 5  # Adjust cost as needed

def get_print_jobs():
    """Retrieves print jobs with details including copies per job."""
    try:
        printer_handle = win32print.OpenPrinter(PRINTER_NAME)
        jobs = win32print.EnumJobs(printer_handle, 0, -1, 1)  # Fetch all print jobs
        win32print.ClosePrinter(printer_handle)
        return jobs
    except pywintypes.error:
        print("⚠️ Warning: Could not retrieve print jobs.")
        return []

def extract_copies(job_id):
    """Extracts the number of copies from the RAW print job data."""
    try:
        printer_handle = win32print.OpenPrinter(PRINTER_NAME)
        job_info = win32print.GetJob(printer_handle, job_id, 2)  # Get detailed job info
        win32print.ClosePrinter(printer_handle)

        if "pPrintProcessorParameters" in job_info:
            raw_data = job_info["pPrintProcessorParameters"]
            if isinstance(raw_data, bytes):
                raw_text = raw_data.decode(errors="ignore")
                for line in raw_text.split("\n"):
                    if "Copies=" in line:
                        return int(line.split("=")[1].strip())  # Extract copies
    except Exception as e:
        print(f"Error extracting copies: {e}")
    
    return 1  # Default to 1 copy if not found

def compute_cost():
    """Computes printing cost, considering both pages and copies per job."""
    jobs = get_print_jobs()
    total_sheets = 0

    for job in jobs:
        num_pages = job.get("TotalPages", 0)  # Pages per job
        num_copies = extract_copies(job["JobId"])  # Extract actual copies
        total_sheets += num_pages * num_copies  # Multiply pages by copies
        print(f"Job {job['JobId']}: {num_pages} pages × {num_copies} copies")

    return total_sheets * BW_COST_PER_PAGE  # Compute total cost
...
def wait_for_print_completion():
    global inserted_coins, total_cost, remaining_paper  
    ...
    while True:
        ...
        if not jobs:
            ...
            # Send transaction to Flask server
            transaction_data = {
                "date": time.strftime("%Y-%m-%d %H:%M:%S"),
                "pages": pages_used,
                "cost": total_cost
            }
            requests.post("http://127.0.0.1:5000/add_transaction", json=transaction_data)

            log_message(f"✅ Printed {pages_used} pages. Cost: ₱{total_cost}. Paper left: {remaining_paper} sheets.")
            update_paper_display()

            # Send paper update to Flask server
